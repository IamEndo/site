    ...
    <div className="max-w-4xl">
      ...
      <section className="mb-12">
        ...
        <div className="space-y-4">
          ...
          >
            <div className="mt-3 flex items-center gap-3 p-3 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800">
              <ToggleRight className="w-5 h-5 text-green-600 dark:text-green-400" />
              <span className="text-sm text-green-800 dark:text-green-300">Privacy Mode: Enabled</span>
            </div>
          </Step>
        </div>
      </section>
...
function RequirementCard({ title, items, works }: { title: string; items: string[]; works: boolean }) {
  return (
    <div className={`p-4 rounded-lg border ${works ? 'border-green-200 dark:border-green-800 bg-green-50/50 dark:bg-green-950/20' : 'border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900'}`}>
      <h4 className={`font-medium mb-3 ${works ? 'text-green-700 dark:text-green-300' : 'text-zinc-700 dark:text-zinc-300'}`}>
        {title}
      </h4>
      <ul className="space-y-1">
        {items.map((item, i) => (
          <li key={i} className="flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
            <CheckCircle2 className={`w-4 h-4 ${works ? 'text-green-500' : 'text-zinc-400'}`} />
            {item}
          </li>
        ))}
      </ul>
...
function ModeCard({ icon, title, items, enabled }: { icon: React.ReactNode; title: string; items: string[]; enabled: boolean }) {
  return (
    <div className={`p-4 rounded-lg border ${enabled ? 'border-green-200 dark:border-green-800 bg-green-50/50 dark:bg-green-950/20' : 'border-zinc-200 dark:border-zinc-800'}`}>
      <div className="flex items-center gap-2 mb-3">
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${enabled ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500'}`}>
          {icon}
        </div>
        <h4 className={`font-medium ${enabled ? 'text-green-700 dark:text-green-300' : 'text-zinc-700 dark:text-zinc-300'}`}>
          {title}
        </h4>
      </div>
      <ul className="space-y-1">
