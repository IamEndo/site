...
const PASSIVES: [number, number][] = [
  ...
  // extra LDO-field parts (photo: the regulator area is BUSY)
  [-21.5, 11.3], [-28.5, 11.5], [-34, 16.5], [-29, -1],
  [-36, -8], [-28, -1.5],
];

// Larger 0805 MLCC caps — brown ceramic bodies, clearly bigger than the
// 0603 field parts in the photo (regulator in/out caps + C7/C8)
const PASSIVES_BIG: [number, number][] = [
  // regulator in/out caps: C17 C16 above U7, C3 C2 above U1
  [-21, 15.8], [-23.3, 15.8], [-21, 7.3], [-23.3, 7.3],
  // C15 / C1 left of the LDO pair
  [-28.6, 15.8], [-28.6, 7.3],
  // between the two LDOs
  [-25.5, 17.4], [-25.5, 9.0],
...
const COLOR = {
  ...
  pcbHi: "#e9b82e",
  pcbCopperGnd: "#a8801c",
  // Traces: clearly darker olive-green than the mask (photo-matched —
  // mask ≈ #d2961e, trace core ≈ #968228) with a dark edge outline
  pcbCopperTrace: "#8e7c20",
  pcbCopperPwr: "#807014",
  pcbTraceEdge: "rgba(95, 72, 8, 0.58)",
  pcbVia: "#3a280a",
  ...
  silkDark: "rgba(68, 58, 24, 1)", // etched-dark silk (board name, front label)
  rubber: "#0f0f12",
  ldrBody: "#2a2a30",
  smdYellow: "#a1794b",
  smdBlack: "#26262a",
...
// ─────────────────────────────────────────────────────────────────────
export function usePcbBackComposite(): THREE.CanvasTexture | null {
  return React.useMemo(() => {
    ...
    baseGrad.addColorStop(0.6, COLOR.pcb);
    baseGrad.addColorStop(1, "#c8930f");
    ctx.fillStyle = baseGrad;
    ctx.fillRect(0, 0, W, H);

    // ── Layer 1: very subtle tonal variation ─────────────────────────
    // The real mask is an EVEN bright yellow; only faint copper-density
    // shading shows through. (An earlier heavy blotch pass darkened the
    // mask below the trace colour and inverted the trace contrast.)
    ctx.save();
    for (let i = 0; i < 70; i++) {
      const r = prng(i + 13);
      const r2 = prng(i + 97);
      ...
      const shade =
        i % 3 === 0
          ? "rgba(245, 218, 110, 0.045)"
          : "rgba(150, 108, 14, 0.05)";
    ...
    const trace = (
      ...
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      // Photo traces read chunkier than nominal — scale all widths up
      const w2 = width * 1.3;
      ctx.strokeStyle = COLOR.pcbTraceEdge;
      ctx.lineWidth = PX * (w2 + 0.14);
      path();
      ctx.stroke();
    ...
    const pour = (pts: [number, number][]) => {
      ...
      for (let i = 1; i < pts.length; i++)
        ctx.lineTo(cx(pts[i][0]), cy(pts[i][1]));
      ctx.closePath();
      ctx.fillStyle = "rgba(110, 95, 20, 0.13)";
      ctx.fill();
      ctx.strokeStyle = "rgba(80, 66, 12, 0.45)";
    ...
    // 45°-constrained polyline walker. Every real trace on this board
    // runs at 0°/45°/90° — moves are [sx, sz, len] with sx/sz ∈ {-1,0,1};
    // a diagonal move advances BOTH axes by len (so it renders at 45°).
    const walk = (
      x0: number,
      z0: number,
    ...
    const espL = ESP.x - ESP.w / 2; // module inner (left) edge, x ≈ 18.5

    // 5V: USB-C → LDO pair (U7/U1); micro-USB and P1 join the same rail
    walk(-41.5, POS.usbc.z, [[1, 0, 5], [1, 1, 6], [1, 0, 1.9]], 1.0, COLOR.pcbCopperPwr);
    walk(-30.5, 4.6, [[0, 1, 6.4], [1, 1, 2.1]], 0.9, COLOR.pcbCopperPwr);
    walk(-41, POS.musb.z, [[1, 0, 3], [1, 1, 9.8]], 0.9, COLOR.pcbCopperPwr);
    walk(POS.p1.x + 1.5, POS.p1.z, [[1, 0, 5], [1, -1, 4.2], [1, 0, 2]], 0.8, COLOR.pcbCopperPwr);

    // 3V3 out of U7 → ESP module (the widest run on the board)
    walk(POS.u7.x + 2.9, POS.u7.z, [[1, -1, 5], [1, 0, 25], [1, -1, 3], [1, 0, 8]], 1.2, COLOR.pcbCopperPwr);
    // 3V3 branch down to U6 (squeezes between the S1/S5 through-holes)
    walk(POS.u1.x + 2.9, POS.u1.z, [[0, -1, 9.1], [1, -1, 1]], 0.8);
    // 3V3 branch up to CH340
    walk(10.5, 5.1, [[0, 1, 8], [1, 1, 3.5], [1, 0, 5], [0, 1, 2.3]], 0.7);

    // GND rail along the board centre
    walk(-40, -3, [[1, 0, 25], [0, -1, 5], [1, 0, 35]], 0.95, COLOR.pcbCopperPwr);

    // USB D+/D− → CH340: the long differential pair up the left side
    for (let i = 0; i < 2; i++) {
      walk(
        -41.5,
        -0.6 - i * 0.7,
    ...
    // micro-USB D pair joins the same corridor
    walk(-41, POS.musb.z + 0.9, [[1, 0, 2], [1, 1, 9.8]], 0.35);
    walk(-41, POS.musb.z + 1.6, [[1, 0, 2.7], [1, 1, 9.8]], 0.35);

    // CH340 TX/RX → ESP UART (straight drop onto the module edge)
    walk(POS.ch340.x - 0.6, POS.ch340.z - 2.5, [[0, -1, 6.3]], 0.38);
    walk(POS.ch340.x + 0.6, POS.ch340.z - 2.5, [[0, -1, 6.3]], 0.38);

    // ESP IO0 → BOOT, EN → RESET (straight drops from the module edge)
    walk(POS.boot.x, ESP.z + ESP.l / 2, [[0, 1, 7.6]], 0.38);
    walk(POS.reset.x, ESP.z + ESP.l / 2, [[0, 1, 7.6]], 0.38);

    // ESP GPIO → TF card lanes (parallel bus with a 45° jog)
    for (let i = 0; i < 5; i++) {
      walk(
        espL,
        -1.8 - i * 1.1,
    ...
    // ESP GPIO → LCD fan-out: a tidy parallel bus that runs down the
    // board centre and dives into vias (feeds the front-side flex).
    // Photo: FEWER, BOLDER, well-spaced runs — not a dense thin bus.
    for (let i = 0; i < 6; i++) {
      walk(
        espL,
        6.5 - i * 1.3,
    ...
    // ESP → P3 (Extended IO, straight drops to the -Z edge)
    for (let i = 0; i < 4; i++) {
      const tx = POS.p3.x + (i - 1.5) * 1.9;
      walk(tx, ESP.z - ESP.l / 2, [[0, -1, 10.4]], 0.32);
    }
    // ESP → CN1 (I2C — 45° drop from the module's lower-left corner)
    for (let i = 0; i < 4; i++) {
      const tx = POS.cn1.x + (i - 1.5) * 1.9;
      const dxi = espL - tx;
      walk(espL, -4.2 - i * 0.8, [[-1, -1, dxi], [0, -1, 15.8 - 4.2 - i * 0.8 - dxi + 4.2]], 0.3);
    }

    // U6 (touch) ↔ ESP + front-flex lanes
    for (let i = 0; i < 4; i++) {
      const zi = -9.3 + i * 1.2;
      walk(POS.u6.x + 2.5, zi, [[1, 0, 8], [1, 1, 4], [1, 0, espL - (POS.u6.x + 2.5) - 12]], 0.3);
      walk(POS.u6.x - 2.5, zi, [[-1, -1, 3], [-1, 0, 5], [-1, -1, 2 + i * 0.5]], 0.3);
    }

    // Amp → Speak connector (parallel pair along the +Z edge)
    walk(POS.amp.x - 2.6, POS.amp.z + 0.8, [[-1, 0, 10], [-1, 1, 2], [-1, 0, 9]], 0.5);
    walk(POS.amp.x - 2.6, POS.amp.z - 0.8, [[-1, 0, 9], [-1, 1, 3], [-1, 0, 10]], 0.5);
    // ESP DAC → amp
    walk(espL, 8.7, [[-1, 0, 3], [-1, 1, 4], [0, 1, 3.5]], 0.4);

    // RGB LED lines from the ESP's bottom edge
    for (let i = 0; i < 3; i++) {
      walk(
        19.5 + i * 1.5,
        ESP.z - ESP.l / 2,
    ...
    // A few short fan-out stubs for visual density — 45°-constrained
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const dir = Math.floor(prng(i + 1303) * 8);
      const sx = [1, 1, 0, -1, -1, -1, 0, 1][dir];
      const sz = [0, 1, 1, 1, 0, -1, -1, -1][dir];
      walk(x, z, [[sx, sz, len]], 0.2 + prng(i + 91) * 0.08);
    }
    ...
    const goldPad = (xMm: number, zMm: number, w: number, h: number) => {
      ...
      grad.addColorStop(0, "#dfe2e6");
      grad.addColorStop(0.5, "#b8bcc2");
      grad.addColorStop(1, "#8e9298");
      ctx.fillStyle = grad;
      ctx.fillRect(
        cx(xMm) - (w * PX) / 2,
        cy(zMm) - (h * PX) / 2,
    ...
    // the ±X sides (photo: pads sit mostly INSIDE the silk outline)
    for (let i = 0; i < 4; i++) {
      const z = POS.u4.z - 1.9 + i * 1.27;
      goldPad(POS.u4.x + 2.7, z, 1.3, 0.55);
      goldPad(POS.u4.x - 2.7, z, 1.3, 0.55);
    }
    ...
    for (const sx of POS.s135.xs) {
      ...
      ctx.fillRect(
        cx(sx) - PX * 1.3,
        cy(POS.s135.z) - PX * 1.3,
        PX * 2.6,
        PX * 2.6
      );
      // dull tinned annulus (photo: subdued ring, modest dark hole —
      // the earlier big-donut version read twice the real size)
      ctx.fillStyle = "#a8abb0";
      ctx.beginPath();
      ctx.arc(cx(sx), cy(POS.s135.z), PX * 1.0, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = COLOR.hole;
      ctx.beginPath();
      ctx.arc(cx(sx), cy(POS.s135.z), PX * 0.68, 0, Math.PI * 2);
      ctx.fill();
    }

    // Tiny pad-pairs under SMD passives
    const padPair = (x: number, z: number, s = 1) => {
      goldPad(x - 0.6 * s, z, 0.5 * s, 0.7 * s);
      goldPad(x + 0.6 * s, z, 0.5 * s, 0.7 * s);
    };
    PASSIVES.forEach(([x, z]) => padPair(x, z));
    PASSIVES_BIG.forEach(([x, z]) => padPair(x, z, 1.3));
    ...
    // WHITE silk (photo re-check: it reads the same cream-white as the
    // designators, NOT dark), running along the board's long axis near
    // the left edge. Canvas x maps to board +x directly — no rotation.
    ctx.save();
    ctx.translate(cx(-13), cy(16.2));
    ctx.scale(0.78, 1);
    ctx.font = `500 ${PX * 3.1}px ui-monospace, monospace`;
    ...
    outline(POS.u4.x, POS.u4.z, 6.0, 4.8);
    outline(POS.sd.x, POS.sd.z, 15.6, 15.2);
...
function useEspShieldTexture(): THREE.CanvasTexture | null {
  return React.useMemo(() => {
    ...
    g.addColorStop(0, "#94979d");
    g.addColorStop(0.35, "#b7bac0");
    g.addColorStop(0.6, "#9ea1a7");
    g.addColorStop(1, "#7e8187");
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, W, H);
    // brushed streaks
    for (let y = 0; y < H; y += 2) {
    ...
    ctx.strokeStyle = "rgba(70, 73, 80, 0.3)";
    ctx.lineWidth = W * 0.01;
    ctx.strokeRect(W * 0.035, H * 0.035, W * 0.93, H * 0.93);
    ctx.strokeStyle = "rgba(225, 228, 232, 0.28)";
    ...
    // Small cert / FCC lines — tiny on the real can, don't let them shout
    ctx.font = `${W * 0.034}px ui-sans-serif, sans-serif`;
    ctx.fillText("FCC ID:2AC7Z-ESPWROOM32", W * 0.5, H * 0.40);
    ctx.font = `${W * 0.03}px ui-sans-serif, sans-serif`;
    ctx.fillText("IC:21098-ESPWROOM32", W * 0.5, H * 0.465);
    ctx.fillText("CMIIT ID:2016DP5670", W * 0.5, H * 0.525);

    // QR patch — sits just under the cert lines, right-of-centre
    // (photo: ~55-65% height, NOT hugging the bottom edge). Dark modules
    // etched straight into the metal (no white label behind the code)
    const qx = W * 0.6;
    const qy = H * 0.55;
    const qs = W * 0.28;
    const N = 17;
    ...
    // Batch code left of the QR, ~70% height (photo)
    ctx.fillStyle = etch;
    ctx.font = `600 ${W * 0.055}px ui-monospace, monospace`;
    ctx.fillText("XX8R69", W * 0.26, H * 0.72);

    // Pin-1 dimple — small dark punched dot near the bottom-left corner
    // of the can (clearly visible in the photo)
    ctx.fillStyle = "#2e3036";
    ctx.beginPath();
    ctx.arc(W * 0.12, H * 0.9, W * 0.022, 0, Math.PI * 2);
    ctx.fill();
...
function ICBlock({
  ...
  color?: string;
  pinDot?: boolean;
  /** which ±Z corner carries the pin-1 dot (photo-checked per package) */
  pinDotZ?: -1 | 1;
  /** gull-wing legs per side (0 = none); protrude off ±pinAxis */
  pins?: number;
  pinAxis?: "x" | "z";
}) {
  const legs: React.ReactNode[] = [];
  if (pins > 0) {
    const along = pinAxis === "z" ? w : h; // body extent the row runs along
    const across = pinAxis === "z" ? h : w; // extent the legs stick out of
    const rowSpan = along - 1.2;
    for (let i = 0; i < pins; i++) {
      const t = pins === 1 ? 0 : -rowSpan / 2 + (i * rowSpan) / (pins - 1);
      for (const s of [-1, 1]) {
        const off = s * (across / 2 + 0.35);
        const lx = pinAxis === "z" ? x + t : x + off;
        const lz = pinAxis === "z" ? z + off : z + t;
        legs.push(
          <mesh
            key={`icleg-${pinAxis}-${i}-${s}`}
            position={[mm(lx), mm(-(PCB_T / 2) - 0.22), mm(lz)]}
          >
            <boxGeometry
              args={[
                mm(pinAxis === "z" ? 0.38 : 0.7),
                mm(0.3),
                mm(pinAxis === "z" ? 0.7 : 0.38),
              ]}
            />
  ...
  return (
    <group>
      ...
      {/* Faint laser-marking lines — the photo ICs all carry pale part
          numbers; two low-contrast streaks along the body's long axis
          sell that at site zoom */}
      {w >= 3 &&
        [-0.35, 0.45].map((off, i) => {
          const alongX = w >= h; // text runs along the longer extent
          const len = (alongX ? w : h) * 0.55;
          return (
            <mesh
              key={`ictext-${i}`}
              position={[
                mm(x + (alongX ? 0 : off)),
                mm(-(PCB_T / 2) - thick - 0.055),
                mm(z + (alongX ? off : 0)),
              ]}
            >
              <boxGeometry
              ...
              />
              <meshStandardMaterial color="#393b42" roughness={0.55} />
            </mesh>
          );
        })}
      {pinDot && (
...
function JstConnector({
  ...
  return (
    <group position={[mm(x), mm(yCenter), mm(z)]} rotation={[0, rotY, 0]}>
      ...
      {/* Bent through-hole legs — thick bright pins out the back of the
          housing, then a solder joint at the board (very prominent in
          the photo: the legs read almost as big as the housing details) */}
      {Array.from({ length: pins }).map((_, k) => {
        const lx = -((pins - 1) / 2) * pitch + k * pitch;
        return (
          <group key={`leg-${k}`}>
            <mesh position={[mm(lx), mm(H / 2 - 0.35), mm(-D / 2 - 0.6)]}>
              <boxGeometry args={[mm(0.44), mm(0.44), mm(1.5)]} />
              {legMat}
            </mesh>
            <mesh position={[mm(lx), mm(H / 2 - 0.1), mm(-D / 2 - 1.2)]}>
              <cylinderGeometry args={[mm(0.52), mm(0.34), mm(0.4), 10]} />
              {legMat}
            </mesh>
          </group>
        );
...
function MicroUsb({ x, z }: { x: number; z: number }) {
  ...
  return (
    <group position={[mm(x), mm(-(PCB_T / 2) - T / 2 - 0.2), mm(z)]} castShadow>
      ...
      {/* Two stamped retention dimples on the shell face near the mouth
          (the signature micro-B detail in the photo) */}
      {[-1.6, 1.6].map((dz) => (
        <mesh
          key={`mu-dimple-${dz}`}
          position={[mm(-W / 2 + 1.6), mm(-T / 2 - 0.02), mm(dz)]}
          rotation={[Math.PI / 2, 0, 0]}
        >
          <cylinderGeometry args={[mm(0.55), mm(0.55), mm(0.04), 14]} />
          <meshPhysicalMaterial
            color="#5f6268"
            roughness={0.3}
            metalness={0.9}
  ...
  return (
    <group position={[mm(x), 0, mm(z)]}>
      ...
      {/* Raised plunger — a LARGE bright metal rounded bar spanning most
          of the body (photo: a big silver cap standing tall off the
          base, with strong highlights) */}
      <mesh position={[0, mm(-(PCB_T / 2) - BH - 0.5), 0]} castShadow>
        <boxGeometry args={[mm(2.3), mm(1.0), mm(4.3)]} />
        <meshPhysicalMaterial
          color="#dde0e4"
          roughness={0.22}
          metalness={0.85}
      ...
      <mesh position={[0, mm(-(PCB_T / 2) - BH - 1.02), 0]}>
        <boxGeometry args={[mm(1.7), mm(0.06), mm(3.6)]} />
        <meshStandardMaterial color="#f4f5f7" roughness={0.28} metalness={0.5} />
      </mesh>
...
function Sot223({ x, z }: { x: number; z: number }) {
  ...
  return (
    <group position={[mm(x), 0, mm(z)]}>
      ...
      {/* Tab — big bright tinned pad, one of the shiniest things in the
          photo's LDO region */}
...
// ─────────────────────────────────────────────────────────────────────
// The photo shows a HIGH-CONTRAST steel lid: gunmetal base with one wide
// bright specular band sweeping across it. The studio env is too weak to
// produce that, so the band is baked into a lid texture.
function useSdLidTexture(): THREE.CanvasTexture | null {
  return React.useMemo(() => {
    if (typeof document === "undefined") return null;
    const S = 256;
    const c = document.createElement("canvas");
    ...
    g.addColorStop(0, "#6d7076");
    g.addColorStop(0.38, "#9fa2a8");
    g.addColorStop(0.55, "#dfe2e6");
    g.addColorStop(0.72, "#94979d");
    g.addColorStop(1, "#5f6268");
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, S, S);
    // faint brushed streaks along the draw direction
    for (let i = 0; i < S; i += 3) {
      ctx.globalAlpha = 0.05;
      ctx.fillStyle = i % 6 === 0 ? "#eceef1" : "#54575c";
      ctx.fillRect(0, i, S, 1);
    }
    ctx.globalAlpha = 1;
    // stamped fold seams near the two side edges
    ctx.strokeStyle = "rgba(50, 53, 58, 0.5)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(S * 0.08, 0);
    ctx.lineTo(S * 0.08, S);
...
function MicroSd({ x, z }: { x: number; z: number }) {
  ...
  return (
    <group>
      {/* Shell — light brushed stainless (macro photo re-check: the slot
          reads clearly SILVER with warm reflections, not gunmetal) */}
      <mesh
      ...
      {/* Lid sheet — baked specular-band texture (photo: gunmetal with
          one wide bright reflection sweep) */}
      ...
      {Array.from({ length: 8 }).map((_, k) => (
        ...
        >
          <boxGeometry args={[mm(0.7), mm(0.32), mm(1.3)]} />
          {brightMat}
        </mesh>
      ))}
    ...
    <group>
      ...
      {Array.from({ length: 10 }).flatMap((_, i) => {
        ...
        return [-1, 1].map((s) => (
          ...
          >
            <cylinderGeometry
              args={[mm(0.26), mm(0.26), mm(subT + 0.06), 10]}
            />
            <meshPhysicalMaterial
              color="#b9bcc2"
              roughness={0.42}
      ...
      {/* Meander antenna on the overhanging tab — the classic WROOM
          snake: fingers along X (the overhang direction) joined by
          alternating tip/root bars. Slightly glossy metal over the
          matte tab so the pattern reads at a glance like the photo. */}
      {(() => {
        const traceY = -(PCB_T / 2) - subT - 0.05;
        // Photo: the meander reads GOLD/bronze (exposed plated copper on
        // the black tab), bright and unmistakable — not grey metal
        const antMat = (
          <meshPhysicalMaterial
            color="#c49a30"
            roughness={0.3}
        ...
        const FINGERS = 6;
        const fw = 1.0; // finger width (Z)
        const span = l - 4.2; // total comb span across the tab
        const pitch = span / (FINGERS - 1);
        const fingerL = ESP_ANT_LEN - 2.6;
        const parts: React.ReactNode[] = [];
        for (let i = 0; i < FINGERS; i++) {
          const fz = z - span / 2 + i * pitch;
          parts.push(
            <mesh
              key={`ant-f-${i}`}
              position={[mm(antX0 + 1.2 + fingerL / 2), mm(traceY), mm(fz)]}
            >
              <boxGeometry args={[mm(fingerL), mm(0.07), mm(fw)]} />
              {antMat}
            </mesh>
          );
        }
        // alternating joins: tip end for even pairs, root end for odd —
        // this is what makes it read as a meander instead of a comb
        for (let i = 0; i < FINGERS - 1; i++) {
          const fz = z - span / 2 + i * pitch + pitch / 2;
          const atTip = i % 2 === 0;
          const jx = atTip
            ? antX0 + 1.2 + fingerL - fw / 2
            : antX0 + 1.2 + fw / 2;
          parts.push(
            <mesh key={`ant-j-${i}`} position={[mm(jx), mm(traceY), mm(fz)]}>
              <boxGeometry args={[mm(fw), mm(0.07), mm(pitch + fw)]} />
              {antMat}
            </mesh>
          );
        }
        // feed stub from the shield end into the first finger
        parts.push(
          <mesh
            key="ant-feed"
            position={[mm(antX0 + 0.5), mm(traceY), mm(z - span / 2)]}
          >
            <boxGeometry args={[mm(1.6), mm(0.07), mm(fw)]} />
            {antMat}
          </mesh>
        );
        return parts;
    ...
    <group>
      ...
      {MOUNT_HOLES.map(([x, z], i) => (
        <mesh
          ...
          rotation={[Math.PI / 2, 0, 0]}
        >
          <ringGeometry args={[mm(1.5), mm(2.1), 32]} />
          <meshPhysicalMaterial
            color="#c2c5ca"
            roughness={0.35}
            metalness={0.9}
      ...
      {/* Black border around active area.
          NOTE on the stack-up: every layer here gets ≥0.04mm of air above
          the one below — coplanar faces z-fight (the film rim's top used
          to sit exactly at the LCD plane's height → stripe flicker). */}
      <mesh position={[mm(TFT_CX), mm(TFT_TOP_Y + 0.05), 0]}>
      ...
      <mesh position={[mm(TFT_CX), mm(TFT_TOP_Y + 0.1), 0]}>
        <boxGeometry args={[mm(SCR_W + 3.2), mm(0.04), mm(SCR_H + 3.2)]} />
        <meshStandardMaterial color="#84888f" roughness={0.42} metalness={0.2} />
      </mesh>

      {/* Active LCD surface — powered-off grey-mirror, not pure black */}
      <mesh
        position={[mm(TFT_CX), mm(TFT_TOP_Y + 0.16), 0]}
        rotation={[-Math.PI / 2, 0, 0]}
      >
      ...
      <mesh
        position={[mm(TFT_CX), mm(TFT_TOP_Y + 0.24), 0]}
        rotation={[-Math.PI / 2, 0, 0]}
      >
      ...
      {/* (The teal tab that used to fold over the +Z edge here was a
          screen-protector pull tab visible in the reference photo — not
      ...
      <ICBlock x={POS.ch340.x} z={POS.ch340.z} w={4.4} h={3.2} thick={1.2} pins={8} pinAxis="z" />
      ...
      <ICBlock x={POS.u6.x} z={POS.u6.z} w={4} h={7} thick={1.3} pins={8} pinAxis="x" pinDotZ={1} />
      <SopPins x={POS.u6.x} z={POS.u6.z} count={8} span={7} offset={2.55} axis="z" />

      {/* U5 — speaker amp SOP-8 */}
      <ICBlock x={POS.amp.x} z={POS.amp.z} w={4.2} h={3.4} thick={1.2} pins={4} pinAxis="z" />
      ...
      <group position={[mm(POS.rgb.x), 0, mm(POS.rgb.z)]}>
        ...
        {/* Quadrant seams — SUBTLE pale-olive dividers (photo: the cross
            barely reads; the mask tint shows through the translucent
            package, it is NOT a bold dark cross) */}
        <mesh position={[0, mm(-(PCB_T / 2) - 1.56), 0]}>
          <boxGeometry args={[mm(0.45), mm(0.04), mm(2.9)]} />
          <meshStandardMaterial color="#b4b8a8" roughness={0.45} />
        </mesh>
        <mesh position={[0, mm(-(PCB_T / 2) - 1.56), 0]}>
          <boxGeometry args={[mm(2.9), mm(0.04), mm(0.45)]} />
          <meshStandardMaterial color="#b4b8a8" roughness={0.45} />
        </mesh>
        {/* LED dies — three tiny dark specks + bond dots at the centre
            (what actually draws the eye in the macro photo) */}
        {[-0.55, 0, 0.55].map((dx) => (
          <mesh
            key={`die-${dx}`}
            position={[mm(dx), mm(-(PCB_T / 2) - 1.58), mm(0.1)]}
          >
            <boxGeometry args={[mm(0.34), mm(0.03), mm(0.34)]} />
            <meshStandardMaterial color="#3a3d42" roughness={0.4} />
          </mesh>
        ))}
      ...
      {/* SOD-123 schottky diode (D1) — black body with the white cathode
          band clearly visible in the photo */}
      <mesh
      ...
      <mesh
        position={[mm(POS.d1.x), mm(-(PCB_T / 2) - 1.11), mm(POS.d1.z - 0.85)]}
      >
        <boxGeometry args={[mm(1.4), mm(0.03), mm(0.5)]} />
        <meshStandardMaterial color="#e8e9ec" roughness={0.5} />
      </mesh>
      {[-1, 1].map((s) => (
        <mesh
          key={`d1-end-${s}`}
          position={[mm(POS.d1.x), mm(-(PCB_T / 2) - 0.25), mm(POS.d1.z + s * 1.55)]}
        >
          <boxGeometry args={[mm(1.0), mm(0.4), mm(0.6)]} />
          <meshStandardMaterial color="#dcdee2" roughness={0.32} metalness={0.8} />
        </mesh>
      ))}
      ...
      {PASSIVES.map(([x, z], i) => (
        <SmdPassive
          ...
          z={z}
          color={
            // the twin resistor columns beside P3 are ALL black (photo)
            x > 20 && z < -10
              ? COLOR.smdBlack
              : i % 3 === 0
              ? COLOR.smdYellow
      ...
      {/* 0805 brown MLCC caps around the regulators / CH340 */}
      {PASSIVES_BIG.map(([x, z], i) => (
        <SmdPassive
          key={`pvb-${i}`}
          x={x}
          z={z}
          color={COLOR.smdYellow}
          size="0805"
        />
      ))}
    </group>
